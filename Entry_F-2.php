...
function f1()
{
    ...
    $qs = "SELECT * FROM parking WHERE floor = 2";
        ...
        if ($vhno !== "" && $mono !== "") {
            if (preg_match('/^[A-Z]{2}[0-9]{2}[A-Z]{2}[0-9]{4}$/', $vhno)) {
                if (preg_match('/^[0-9]{10}$/', $mono)) {
                    // Insert Data into DB
                    $sql = "INSERT INTO `parking` (`Operator`,`floor`,`slot_no`,`vh_no`,`mobile_no`,`mail`,`entrytime`) VALUES ('$operator','2','$slot_no', '$vhno','$mono','$mail', current_timestamp());";
                    ...
                    header('location: ./Entry_F-2.php');
                    return;
                } else { ?>
                    <div class="alert alert-danger alert-dismissible fade show m-0" role="alert">
                        Please Enter valid <strong>Mobile Number</strong><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                <?php }
            } else { ?>
                <div class="alert alert-danger alert-dismissible fade show m-0" role="alert">
...
<body>
    ...
    <header>
        <nav>
            <ul>
                <li><a href="./Entry_F-0.php">Floor-0</a></li>
                <li><a href="./Entry_F-1.php">Floor-1</a></li>
                <li><a href="./Entry_F-2.php" class="active">Floor-2</a></li>
            </ul>
        </nav>
    </header>
    ...
    <div class="contain" style="display: flex; align-items: center; flex-direction: column;">
        ...
        include('./parking/floor2-display.php') ?>
    </div>
